...
function buildBreakdownGroups(order = {}, cfg = {}, capability = null) {
  ...
  const isApiShop = supportsApiFeePlatform && (capabilityMode === 'api_active' || hasAnyApiDetail)
...
export function buildOrderFeePhase1Snapshot(order = {}, cfg = {}, capabilityLookup = new Map()) {
  ...
  const total = roundMoney(apiFeeTotal + apiTaxTotal + internalTotal + estimateTotal)
