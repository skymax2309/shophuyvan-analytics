...
class ShopeeOrderScraper:
    ...
    async def scrape_new_orders(self, page, limits=None, shop_name="default", mode="all"):
        # 🌟 GỌI MODULE QUẢN GIA AUTH (CHUẨN HÓA QUY TẮC 18 - CHIA ĐỂ TRỊ)
        import sys, os
        sys.path.append(os.path.dirname(os.path.abspath(__file__)))
        from shopee_auth import ShopeeAuth
        
        auth_handler = ShopeeAuth(self.log)
        tokens_data = auth_handler.get_api_tokens_data(shop_name)
        ...
        if tokens_data and tokens_data.get("access_token"):
            ...
            # Chạy thử API
            api_result = await self.scrape_by_api(token, shop_name, limits, mode)
            ...
            if api_result is False:
                ...
                new_token = auth_handler.refresh_api_token(tokens_data, shop_name)
                
                if new_token:
                    self.log("🚀 Đang khởi động lại luồng lấy Đơn hàng bằng Token mới nóng hổi...")
                    return await self.scrape_by_api(new_token, shop_name, limits, mode)
                else:
                    self.log("❌ Làm mới Token thất bại. Bác vui lòng quét mã QR lại trên ứng dụng!")
                    return [] # Trả về list rỗng, TUYỆT ĐỐI không bật Chrome dự phòng
    ...
    async def scrape_by_api(self, token, shop_name, limits, mode):
        ...
        while True:
            url_list = auth_handler.sign_api(path_list, token, shop_id)
            ...
            try:
                ...
                if data.get("error"):
                    ...
                    return False # 🌟 TRẢ VỀ FALSE ĐỂ ĐÁNH THỨC CƠ CHẾ AUTO REFRESH
        ...
        for i in range(0, len(all_order_sns), 50):
            ...
            url_detail = auth_handler.sign_api(path_detail, token, shop_id)
            sn_str = ",".join(chunk)
